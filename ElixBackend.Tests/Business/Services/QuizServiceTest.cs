...
public class QuizServiceTest
{
    ...
    public async Task SubmitQuizAsync_TenAnswers_EightCorrect_AddsPoints_EchoesResult()
    {
        ...
        // userPointService.AddUserAsync will be called with 8 points
        _userPointServiceMock
            .Setup(s => s.AddUserAsync(It.Is<UserPointDto>(p => p.UserId == userId && p.CategoryId == categoryId && p.Points == 8)))
        ...
        _userPointServiceMock.Verify(s => s.AddUserAsync(It.Is<UserPointDto>(p => p.Points == 8)), Times.Once);
    }
    ...
    public async Task SubmitQuizAsync_LessThanTen_NoPoints()
    {
        ...
        Assert.That(result, Is.Not.Null);
        _userPointServiceMock.Verify(s => s.AddUserAsync(It.IsAny<UserPointDto>()), Times.Never);
    }
}